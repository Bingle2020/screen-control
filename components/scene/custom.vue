<template>
  <div
    ...
    ref="box"
  >
    <Modal v-model="modal1" title="保存场景" @on-ok="ok" @on-cancel="cancel">
      场景名称
      <input class="sInput" type="text" placeholder="请输入场景名称" v-model="sceneName" />
    </Modal>
    <!-- 配置盒子 -->
    <modules @addlayer="addset"></modules>
    <!-- 保存场景按钮 -->
    <button disabled class="savebtn" style="color: gray" v-if="turnof">保存</button>
    <Button type="primary" @click="modal1 = true" v-show="!display" v-else>保存</Button>
...
export default {
  ...
  methods: {
    ...
    ok() {
      ...
      let obj = {}
      obj.id = 5
      obj.NO = new Date().getTime()
      obj.name = this.sceneName
      obj.original = {}
      obj.original.W = this.$refs.grids.offsetWidth
      obj.original.H = this.$refs.grids.offsetHeight
      obj.data = {}
      obj.data.grids = [...this.grids]
      obj.data.revert = [...this.revert]
      this.$store.commit('addScene', obj)
      this.sceneName = ''
      //保存成功提示
      // this.saveLayer()
      // this.saveExtraData()
    ...
    initGrids() {
      ...
      // const w = W / C
      // const h = H / R
      const w = parseFloat((W / C).toFixed(6))
      const h = parseFloat((H / R).toFixed(6))
    ...
    initDatas() {
      var grids, revert
      axios.get('/Elevator/Module/LoadModulePage?sceneNum=03').then(res => {
        // console.log(res.data)
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          if (data[i].moduleType == 'grids') {
            ...
            if (grids.length > 0) {
              this.grids = [...grids]
              // console.log('前一次有保存数据,加载数据', this.grids)
            } else {
              //如果前一次没有数据则重新建立数据
              this.initGrids()
              // console.log('前一次没有数据,执行网格初始化', this.grids)
            }
          }
          if (data[i].moduleType == 'revert') {
            ...
            if (revert.length > 0) {
              ...
              //如果前一次没有数据则重新建立数据
              this.revert = []
              // console.log('revert初始化', this.revert)
            }
          }
        }
      })
    ...
    saveData() {
      ...
      param.Items.push(
        {
          ModuleType: 'grids',
          ModuleContent: JSON.stringify(this.grids),
          ModuleLocation: '01',
          OrderNo: 1
        },
        {
          ModuleType: 'revert',
          ModuleContent: JSON.stringify(this.revert),
          ModuleLocation: '02',
          OrderNo: 2
        }
      )
    ...
    //计算left值
    getLeft(n, c, w) {
      return parseFloat(((n - 1) % c * w).toFixed(6))
      // return (n - 1) % c * w
    },
    //计算top值
    getTop(n, c, h) {
      return parseFloat((parseInt((n - 1) / c) * h).toFixed(6))
      // return parseInt((n - 1) / c) * h
    },
    //计算color值
    getColor() {
