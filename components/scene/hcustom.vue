<template>
  <div
    ...
    ref="box"
    :style="{width: wid + 'px',height: high + 'px'}"
  >
    <div
      class="grids"
      :style="{width: wid + 'px',height: high + 'px'}"
    ...
    >
      <div
        ...
        @mousedown.self="pressdown"
      >
        <i class="iconfont icon-peizhiguanli" v-if="item.merge" @click="screenset(item.id)"></i>
        <sortableTable
    ...
    <div class="setbox" v-if="setDiv">
      <i class="iconfont icon-guanbi" @click="shutdown"></i>
    </div>
...
export default {
  ...
  props: ['row', 'col', 'wid', 'high', 'eleft', 'etop', 'actId'],
  components: {
    sortableTable: () => import('@/components/sortableTable/customplugin.vue')
  },
  mounted() {
     this.initGrids();
   // this.initDatas()
  ...
  methods: {
    ...
    initGrids() {
      ...
      const W = Number(this.wid)
      const H = Number(this.high)
      const N = R * C
      const w = W / C
      const h = H / R
      var arr = []
    ...
    pressdown(e) {
      ...
      //按下时的起始坐标,并赋值给框选盒子
      this.startX = (e.x || e.clientX) - lfa - this.eleft
      this.startY = (e.y || e.clientY) - tfa - this.etop
    ...
    pressmove(e) {
      ...
      if (this.isSelect) {
        ...
        //移动坐标
        let _x = (e.x || e.clientX) - lfa - this.eleft
        let _y = (e.y || e.clientY) - tfa - this.etop
        //改变选框的属性值
        this.selDiv = {
    ...
    mouserelease(e) {
      ...
      for (let i = 0, len = this.grids.length; i < len; i++) {
        if (this.grids[i].seled && this.isSelect) {
          ...
          //鼠标松开时坐标
          let x = (e.x || e.clientX) - lfa - this.eleft + 20
          let y = (e.y || e.clientY) - tfa - this.etop + 20
