  ...
  methods: {
    ...
    initDatas() {
      ...
      axios.get('/Elevator/Module/LoadModulePage?sceneNum=08').then(res => {
        ...
        for (var i = 0; i < data.length; i++) {
          ...
          //转化尺寸
          this.isDataPercentage()
        })
    },
    //数据是否百分化?
    isDataPercentage(){
      let W = this.original.W
      let H = this.original.H
    ...
    getLayD() {
      axios.get('/Elevator/Module/LoadModulePage?sceneNum=09').then(res => {
        ...
        this.$store.commit(
          'setLayD',
          JSON.parse(datas.moduleContent)
        )
      })
    },
    //保存布局和数据
    saveData() {
      ...
      param.Items.push(
        ...
        },
        {
          ModuleType: 'original',
          ModuleContent: JSON.stringify({
            W: this.$refs.grids.offsetWidth,
            H: this.$refs.grids.offsetHeight
          }),
          ModuleLocation: '03',
          OrderNo: 3
        }
